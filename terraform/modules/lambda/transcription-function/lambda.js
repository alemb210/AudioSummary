//Credit to https://github.com/simalexan/s3-lambda-transcribe-audio-to-text-s3
...
exports.handler = (event, context) => {
  const eventRecord = event.Records && event.Records[0],
    ...
    id = context.awsRequestId; //remove once we confirm working
  ...
  const fileUri = `https://${inputBucket}.s3.amazonaws.com/${key}`,
   // jobName = `s3-lambda-audio-transcribe-${id}`;
    jobName = `${path.basename(key, path.extname(key))}`;
